...
// TTT = ritpunten / 2, naar boven afgerond
const TTT_PTS = Object.fromEntries(Object.entries(RIT_PTS).map(([k,v]) => [k, Math.ceil(v/2)]));

function ritPts(pos, isTTT) {
  const s = String(pos||'').trim().toUpperCase();
  if (s==='DNF'||s==='DNS'||s==='') return 0;
  const n = parseInt(s); if (isNaN(n)) return 0;
  const tbl = isTTT ? TTT_PTS : RIT_PTS;
  if (tbl[n]) return tbl[n];
  // Lagere posities ook halveren bij TTT
  if (n<=20) return isTTT ? 5  : 10;
  if (n<=25) return isTTT ? 4  : 8;
  if (n<=50) return isTTT ? 3  : 6;
  if (n<=75) return isTTT ? 2  : 4;
  if (n<=100) return isTTT ? 2 : 3;
  return isTTT ? 1 : 1;
}
...
function _berekenRijen(data, type, uitgereden) {
  ...
  for (let i = 0; i < Math.min(5, data.length); i++) {
    if (data[i]?.some(c => String(c||'').toLowerCase().includes('rider') ||
                           String(c||'').toLowerCase().includes('naam'))) { hIdx = i; break; }
  }
  ...
  const isTTT = type === 'ttt';
  const isRit = type === 'rit' || isTTT;

  // Winnaar voor DNF-bonus (alleen bij rit/ttt)
  let winTeam = null;
  if (isRit) {
  ...
  for (let i = hIdx+1; i < data.length; i++) {
    ...
    if (isRit) {
      // Rit of TTT: ritpunten (bij TTT gehalveerd), dagklassementen normaal
      if (!isDNF && !isDNS) pts_rit = ritPts(rnkRaw, isTTT);
      if (isDNF && winTeam && team === winTeam) pts_bonus = isTTT ? 3 : 5; // ook bonus halveren bij TTT
...
export function switchATab(t) {
  document.querySelectorAll('.atab').forEach(el => el.classList.toggle('active', el.id === 'at-' + t));
  const handlers = {
...
async function renderGebruikers() {
  loading(true);
  const { data: users } = await sb.from('profiles').select('*, user_teams(competitie, ploeg_naam, user_team_renners(renner_id))').eq('is_admin', false).order('naam');
  loading(false);
  const s_norm = getState().settings['normal'] || {}, s_pro = getState().settings['pro'] || {};
  const total = users?.length || 0;
  const rows = (users || []).map(u => {
    const teams = u.user_teams || [];
    const initials = u.naam.split(' ').map(w => w[0]).slice(0,2).join('').toUpperCase();
    if (!teams.length) return `<div class="user-row"><div class="avatar">${initials}</div><div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:500">${u.naam}</div><div style="font-size:11px;color:var(--text2)">${u.email}</div></div><span class="badge">Geen competitie</span></div>`;
    return teams.map(t => {
      const maxR = t.competitie === 'normal' ? s_norm.max_renners : s_pro.max_renners;
      const n = (t.user_team_renners || []).length; const compleet = n === maxR;
      return `<div class="user-row"><div class="avatar">${initials}</div><div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:500">${u.naam}${t.ploeg_naam ? ` <span style="font-weight:400;color:var(--text2)">· ${t.ploeg_naam}</span>` : ''}</div><div style="font-size:11px;color:var(--text2)">${u.email}</div></div><span class="badge ${t.competitie === 'normal' ? 'bg' : 'by'}" style="margin-right:6px">${t.competitie === 'normal' ? 'Normaal' : 'Pro'}</span><span class="badge ${compleet ? 'bg' : 'br'}">${n}/${maxR}${compleet ? ' ✓' : ''}</span></div>`;
    }).join('');
  }).join('');
  document.getElementById('admin-content').innerHTML = `<div class="card"><div class="sh"><div class="card-title" style="margin-bottom:0">Geregistreerde gebruikers</div><span style="font-size:11px;color:var(--text2)">${total} gebruikers</span></div>${rows || '<div style="font-size:13px;color:var(--text2)">Geen gebruikers.</div>'}</div>`;
}

function renderSettingsTab(comp) {
  const s = getState().settings[comp] || {}, pre = comp === 'normal' ? 'n' : 'p', lbl = comp === 'normal' ? 'Normaal' : 'Pro';
  document.getElementById('admin-content').innerHTML = `<div class="card"><div class="card-title">Instellingen — ${lbl}</div>
    ...
    <div class="srow"><div class="slbl">Deadline</div><input type="datetime-local" id="${pre}-dl" value="${s.deadline?s.deadline.slice(0,16):''}" style="width:190px;margin-bottom:0"/><button class="btn btn-sm btn-danger" onclick="clearDeadline('${comp}')">Wis</button></div>
    ...
    <div id="sett-res" style="display:none"></div><button class="btn btn-primary" onclick="saveSettings('${comp}')">Opslaan</button></div>`;
}

window.saveSettings = async function(comp) {
  const pre = comp === 'normal' ? 'n' : 'p';
  const dlVal = document.getElementById(`${pre}-dl`).value;
  const updates = { type: comp, max_renners: parseInt(document.getElementById(`${pre}-mr`).value)||15, budget: parseInt(document.getElementById(`${pre}-b`).value)||1000, max_per_team: parseInt(document.getElementById(`${pre}-mt`).value)||3, deadline: dlVal||null, updated_at: new Date().toISOString() };
...
async function renderKoersenTab() {
  const list = getState().koersen.map(k => {
    ...
    return `<div style="margin-bottom:1rem">
      ...
      <div style="margin:8px 0 0 0">
        <div style="font-size:12px;font-weight:500;color:var(--text2);margin-bottom:5px">Deelnemers importeren voor <strong>${k.naam}</strong></div>
        <div class="alert ai" style="margin-bottom:6px;font-size:12px">Één rennersnaam per rij — exact zoals in de rennersdatabase.<br>De bestaande deelnemerslijst voor <strong>${k.naam}</strong> wordt volledig vervangen.</div>
        <textarea id="deel-csv-${k.id}" class="csv-a" style="height:90px" placeholder="Tadej Pogacar&#10;Remco Evenepoel&#10;..."></textarea>
        <div style="display:flex;gap:7px;margin-bottom:4px;align-items:center;flex-wrap:wrap">
          ...
          <label class="btn btn-sm" style="cursor:pointer">Bestand<input type="file" accept=".csv,.txt" style="display:none" onchange="loadDeelnemersFile(event, '${k.id}', '${k.naam}')"/></label>
  ...
  document.getElementById('admin-content').innerHTML = `<div class="card">
    <div class="sh"><div class="card-title" style="margin-bottom:0">Koersen &amp; deelnemers</div>${getState().koersen.length > 0 ? `<button class="btn btn-sm btn-danger" onclick="clearAlleDeelnemers()">Wis alle deelnemers</button>` : ''}</div>
    <div class="alert ai" style="margin-bottom:.8rem;font-size:12px">Maak een koers aan, importeer daarna de deelnemerslijst. Gebruikers zien dan de filter <strong>"Doet mee aan [koers]"</strong> in de rennersselectie.</div>
    <div style="display:flex;gap:7px;margin-bottom:1rem"><input type="text" id="new-koers" placeholder="bv. Giro d'Italia, Tour de France..." style="margin-bottom:0;flex:1"/><button class="btn btn-primary" onclick="addKoers()">+ Koers toevoegen</button></div>
...
window.loadDeelnemersFile = function(e, koersId, koersNaam) {
  ...
  rd.onload = async ev => { document.getElementById(`deel-csv-${koersId}`).value = ev.target.result.trim(); await _verwerkDeelnemers(koersId, koersNaam, ev.target.result.trim()); };
  rd.readAsText(f);
};
async function _verwerkDeelnemers(koersId, koersNaam, raw) {
  const resEl = document.getElementById(`deel-res-${koersId}`); resEl.style.display = 'none'; loading(true);
  const namen = raw.split('\n').map(l => l.split(';')[0].trim()).filter(l => l.length > 0);
  const norm = n => (n||'').toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  let matched = 0; const notFound = [], rennerIds = [];
  namen.forEach(naam => { const r = getState().renners.find(x => norm(x.naam) === norm(naam)); if (r) { rennerIds.push(r.id); matched++; } else notFound.push(naam); });
  await sb.from('renner_koersen').delete().eq('koers_id', koersId);
  if (rennerIds.length) { const rows = rennerIds.map(rid => ({ renner_id: rid, koers_id: koersId })); for (let i = 0; i < rows.length; i += 500) await sb.from('renner_koersen').insert(rows.slice(i, i+500)); }
  getState().renners.forEach(r => { r.koers_ids = (r.koers_ids||[]).filter(k => k !== koersId); if (rennerIds.includes(r.id)) r.koers_ids.push(koersId); });
  loading(false);
  let msg = `✓ <strong>${matched}</strong> renners gekoppeld aan <strong>${koersNaam}</strong>.`;
  if (notFound.length) msg += `<br><span style="color:var(--text2)">Niet gevonden (${notFound.length}): </span><span style="color:var(--red-text)">${notFound.slice(0,10).join(', ')}${notFound.length>10?` … +${notFound.length-10}`:''}</span>`;
  resEl.className = `alert ${notFound.length && !matched ? 'ad' : 'as'}`; resEl.innerHTML = msg; resEl.style.display = 'block';
  renderKoersenTab();
}
window.clearDeelnemers = async function(koersId, koersNaam) { if (!confirm(`Alle deelnemers van ${koersNaam} verwijderen?`)) return; loading(true); await sb.from('renner_koersen').delete().eq('koers_id', koersId); getState().renners.forEach(r => { if (r.koers_ids) r.koers_ids = r.koers_ids.filter(k => k !== koersId); }); loading(false); renderKoersenTab(); };
window.clearAlleDeelnemers = async function() { if (!confirm('Alle deelnemers van ALLE koersen verwijderen?\nDe koersen zelf blijven bestaan.')) return; loading(true); await sb.from('renner_koersen').delete().neq('renner_id','00000000-0000-0000-0000-000000000000'); getState().renners.forEach(r => { r.koers_ids = []; }); loading(false); renderKoersenTab(); };
window.addKoers = async function() { const naam = document.getElementById('new-koers').value.trim(); if (!naam) return; const { data, error } = await sb.from('koersen').insert({ naam }).select().single(); if (error) { alert(error.message); return; } getState().koersen.push(data); document.getElementById('new-koers').value=''; renderKoersenTab(); };
window.renameKoers = async function(id, naam) { await sb.from('koersen').update({ naam: naam.trim() }).eq('id',id); const k = getState().koersen.find(x => x.id===id); if (k) k.naam = naam.trim(); };
...
async function renderRennersTab() {
  const html = getState().renners.map(r => {
    const koersLabels = (r.koers_ids||[]).map(kid => { const k = getState().koersen.find(x => x.id===kid); return k ? `<span class="koers-tag">${k.naam}</span>` : ''; }).join('');
    return `<div style="padding:7px 0;border-bottom:0.5px solid var(--border)"><div style="display:flex;align-items:center;gap:7px">${jersey(r.ploeg,18)}<div style="flex:1;min-width:0"><span style="font-size:12px;font-weight:500">${r.naam}</span><span style="font-size:11px;color:var(--text2);margin-left:4px">${r.ploeg}</span><div style="margin-top:2px">${koersLabels||'<span style="font-size:10px;color:var(--text2)">Geen koersen gekoppeld</span>'}</div></div><input type="number" style="width:65px;margin-bottom:0;text-align:center;font-size:12px" value="${r.kostprijs}" min="1" max="9999" onchange="updateKostprijs('${r.id}',this.value)" title="Kostprijs"/><span style="font-size:11px;color:var(--text2)">kostprijs</span></div></div>`;
  }).join('');
  document.getElementById('admin-content').innerHTML = `<div class="card"><div class="sh"><div class="card-title" style="margin-bottom:0">Renners &amp; kostprijs</div><div style="display:flex;align-items:center;gap:8px"><span class="badge">${getState().renners.length} renners</span>${getState().renners.length>0?`<button class="btn btn-sm btn-danger" onclick="deleteAlleRenners()">Wis alle renners</button>`:''}</div></div><div class="alert ai" style="margin-bottom:.8rem;font-size:12px">Pas hier de kostprijs per renner aan. Koers-deelname beheer je via het tabblad <strong>Koersen</strong>.</div>${html||'<div style="font-size:13px;color:var(--text2)">Geen renners.</div>'}</div>`;
}
window.updateKostprijs = async function(id, val) { const v = parseInt(val)||1; await sb.from('renners').update({ kostprijs: v }).eq('id',id); const r = getState().renners.find(x => x.id===id); if (r) r.kostprijs = v; };
window.deleteAlleRenners = async function() {
  ...
  getState().renners = []; loading(false); renderRennersTab();
};

// ============================================================
// UITSLAGEN TAB — met Rit / TTT / Einduitslag
// ============================================================
let xlWorkbook = null, xlSelSheets = new Set();

async function renderUitslagenTab() {
  const { data: uitslagen } = await sb.from('uitslagen').select('*, koersen(naam)').order('imported_at', { ascending: false });
  const ovHtml = !(uitslagen?.length)
    ? '<div style="font-size:13px;color:var(--text2)">Nog geen uitslagen.</div>'
    : uitslagen.map(u => `<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:0.5px solid var(--border)">
  ...
  document.getElementById('admin-content').innerHTML = `
    <div class="card">
      ...
      <div class="alert ai" style="margin-bottom:.8rem;font-size:12px">
        <strong>Rituitslag &amp; TTT:</strong> kolommen <code>Rnk · Rider · Team · GC · Points · Berg · Jeugd</code><br>
        <strong>TTT:</strong> ritpunten worden automatisch gehalveerd (naar boven afgerond). Dagklassementen blijven normaal.<br>
        <strong>Einduitslag:</strong> kolommen <code>Rnk · Rider · Team · Points · Berg · Jeugd</code> (Points/Berg/Jeugd = absolute punten uit de cel)
      </div>
      <div class="srow">
      ...
      <div class="srow">
        ...
        <select id="imp-type" style="width:auto;margin-bottom:0" onchange="toggleUitslagType()">
          ...
          <option value="ttt">TTT (Ploegentijdrit) — punten ÷ 2</option>
      ...
      <div class="drop-zone" id="drop-zone" onclick="document.getElementById('xl-input').click()" ondragover="event.preventDefault();this.classList.add('dv')" ondragleave="this.classList.remove('dv')" ondrop="handleDrop(event)">
    ...
    <div class="card" id="sheets-card" style="display:none">
      <div class="sh"><div class="card-title" style="margin-bottom:0">Kies tabbladen</div><div style="display:flex;gap:5px"><button class="btn btn-sm" onclick="selAllSheets(true)">Alles</button><button class="btn btn-sm" onclick="selAllSheets(false)">Geen</button></div></div>
    ...
    <div class="card" id="imp-prog-card" style="display:none">
      ...
      <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2)"><span id="prog-lbl">Verwerken…</span><span id="prog-pct">0%</span></div>
    ...
    <div class="card" style="margin-top:.8rem">
      <div class="sh"><div class="card-title" style="margin-bottom:0">Geïmporteerde uitslagen</div><button class="btn btn-sm btn-danger" onclick="if(confirm('Alles wissen?'))deleteAllUitslagen()">Wis alles</button></div>
...
window.handleDrop = function(ev) { ev.preventDefault(); document.getElementById('drop-zone').classList.remove('dv'); handleFile(ev.dataTransfer.files[0]); };
window.handleFile = function(file) {
  if (!file?.name.endsWith('.xlsx')) { alert('Alleen .xlsx'); return; }
  const rd = new FileReader();
  rd.onload = e => { xlWorkbook = XLSX.read(e.target.result, { type:'array' }); xlSelSheets = new Set(xlWorkbook.SheetNames); document.getElementById('sheets-card').style.display='block'; renderSheetPills(); };
  rd.readAsArrayBuffer(file);
};
function renderSheetPills() { document.getElementById('sheet-pills').innerHTML = xlWorkbook.SheetNames.map(n => `<span class="sheet-pill${xlSelSheets.has(n)?' active':''}" onclick="toggleSheet('${n}')">${n}</span>`).join(''); }
...
window.processSheets = async function() {
  ...
  const log = document.getElementById('prog-log'); log.innerHTML = '';
  const results = [];
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    const { data: uitslag, error: uErr } = await sb.from('uitslagen').upsert({
      koers_id: koersId, sheet_naam: sn, type: type,
      rit_nummer: (type==='rit'||type==='ttt') ? (parseInt(sn.replace(/\D/g,''))||null) : null,
  ...
  getState().allUitslag_rijen = (rijen2||[]).map(r => { const u = (getState().uitslagen||[]).find(x => x.id===r.uitslag_id); return { ...r, type: u?.type||'rit', koers_id: u?.koers_id, sheet_naam: u?.sheet_naam||'' }; });
  ...
  const top = results.map(({ sn, rijen }) => {
    ...
    return `<div class="rit-hdr" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='block'?'none':'block'">
      <span class="badge bg">${koersNaam}</span><span class="badge bb">${TYPE_LABELS[type]||type}</span>
    ...
    <div class="rit-body"><table><thead><tr><th>#</th><th>Renner</th><th>Pos</th><th>Punten</th></tr></thead>
    <tbody>${top3.map((r,i) => `<tr><td class="${['rg','rs','rb'][i]||''}">${i+1}</td><td>${r.renner_naam}</td><td>${r.rnk}</td><td><span class="pts-pill pts-pos">+${r.totaal}</span></td></tr>`).join('')}</tbody></table></div>`;
  }).join('');
...
window.deleteAllUitslagen = async function() {
  ...
  getState().allUitslag_rijen = []; getState().uitslagen = [];
  renderUitslagenTab();
};
...
function renderCsvTab() {
  document.getElementById('admin-content').innerHTML = `<div class="card">
    ...
    <div class="alert ai" style="margin-bottom:.8rem">
      ...
      <code>naam;ploeg;kostprijs</code> &nbsp;·&nbsp; <code>naam;;kostprijs</code> (lege ploeg) &nbsp;·&nbsp; <code>naam;kostprijs</code><br>
      <span style="font-size:11px">Bestaande renners worden bijgewerkt. Nieuwe renners worden toegevoegd.<br><strong>Koers-deelname</strong> stel je apart in via het tabblad <strong>Koersen</strong>.</span>
    </div>
    <textarea class="csv-a" id="csv-in" placeholder="Tadej Pogacar;UAE Team Emirates;95&#10;Jonas Vingegaard;Team Visma | Lease a Bike;92"></textarea>
...
window.loadCsvFile = function(e) { const f = e.target.files[0]; if (!f) return; const rd = new FileReader(); rd.onload = ev => { document.getElementById('csv-in').value = ev.target.result.trim(); }; rd.readAsText(f); };
window.importCsvRenners = async function() {
  const raw = document.getElementById('csv-in').value.trim(); if (!raw) return;
  loading(true);
  const lines = raw.split('\n').map(l => l.trim()).filter(l => l);
  let added=0, updated=0, errors=[], toInsert=[];
  for (const line of lines) {
    const p = line.split(';'); if (p.length < 2) { errors.push(`Ongeldig formaat: ${line}`); continue; }
    const naam = p[0].trim(); if (!naam) { errors.push(`Lege naam: ${line}`); continue; }
    let ploeg, kostprijs;
    if (p.length >= 3 && isNaN(parseInt(p[1].trim()))) { ploeg = p[1].trim(); kostprijs = parseInt(p[2].trim()); }
    else if (p.length >= 2 && !isNaN(parseInt(p[1].trim()))) { ploeg = ''; kostprijs = parseInt(p[1].trim()); }
    else { ploeg = p[1].trim(); kostprijs = parseInt(p[2].trim()); }
    ...
    if (existing) { const { error } = await sb.from('renners').update({ ploeg, kostprijs }).eq('id', existing.id); if (!error) { existing.ploeg=ploeg; existing.kostprijs=kostprijs; updated++; } else errors.push(`Update mislukt voor ${naam}: ${error.message}`); }
    else toInsert.push({ naam, ploeg, kostprijs });
  }
  ...
  if (errors.length) msg += `<br><span style="color:var(--red-text);font-size:11px">${errors.slice(0,5).join('<br>')}${errors.length>5?`<br>… +${errors.length-5} anderen`:''}</span>`;
